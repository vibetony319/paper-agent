...
1. 在模型设置中新增档案，填写服务信息；可选“测试连接”快速确认服务地址可访问。
...
解释和翻译会调用基础对话接口，论文助手会调用工具接口。“测试连接”只检查服务地址是否有 HTTP 响应，不验证密钥、模型名称或具体能力，也不要求模型生成回答；实际不兼容时由对应请求报告错误。vLLM 的工具解析器与聊天模板仍需匹配模型。
