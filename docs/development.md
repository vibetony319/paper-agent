...
请求仍显式携带 `model_profile_id`；模型选择、连接检查、无密钥快照和 usage lease 见 [模型服务与模型档案](model-services.md)。不要把真实密钥写进脚本、示例、SQLite 或日志。
...
vLLM 联调失败时先在模型档案中“测试连接”，确认服务地址可访问；这一步不验证密钥或模型能力。若实际对话失败，再检查服务地址、模型名、鉴权，以及服务端是否为该模型启用了适配的 chat template 与 tool-call parser，并确认 OpenAI-compatible 返回中含可解析的 tool calls。Agent 请求带 `parallel_tool_calls=False`，但 provider 可能忽略它，运行时按批处理并截断到预算上限。
