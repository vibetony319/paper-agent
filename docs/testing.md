...
服务器只替换模型传输层，解析、数据库、模型档案、Agent 工具、Citation Guard、笔记和删除均使用生产代码。每次启动创建独立系统临时目录，正常关闭时清理；强制终止进程可能留下临时目录，但不会写入用户运行数据。每个浏览器用例只删除自己上传的论文。测试 PDF 运行时生成，不提交二进制论文或私人截图。
